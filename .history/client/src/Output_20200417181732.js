...
export default function Output(props) {
    ...
    return (
        ...
        {(data)?(<div className="col offset-s3">{(data.available)?(<div><span>Congratulations! This domain is available</span><br/>
            <span className="col offset-s2"><b>For:</b>{" "+data.currency+" "+data.price}</span></div>):(data.code)?(<div>{data.message}</div>):(<span>Sorry this domain is not Available</span>)}</div>)
